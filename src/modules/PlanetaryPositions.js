...
export class PlanetaryPositions {
  constructor() {
    ...
    this.planets = [
      { body: Astronomy.Body.Mercury, name: "Mercury", color: [169, 169, 169] }, // Gray
      { body: Astronomy.Body.Venus, name: "Venus", color: [255, 230, 170] }, // Pale yellow
      { body: Astronomy.Body.Mars, name: "Mars", color: [255, 100, 70] }, // Red-orange
      { body: Astronomy.Body.Jupiter, name: "Jupiter", color: [255, 220, 180] }, // Pale orange
      { body: Astronomy.Body.Saturn, name: "Saturn", color: [255, 235, 200] }, // Pale yellow-white
    ];
  }
  ...
  calculatePositions(julianDate) {
    ...
    for (const planet of this.planets) {
      try {
        ...
        const equatorial = Astronomy.Equator(planet.body, jsDate, null, true, true);
        ...
        // Use a large distance to place planets on the celestial sphere
        const distance = 1e10; // 10 billion meters - far enough to appear as point sources

        const position = this.equatorialToCartesian(ra, dec, distance);
   ...
   * @returns {Cartesian3} Cartesian position
   */
  equatorialToCartesian(ra, dec, distance) {
    ...
    // Using astronomical convention: RA from X-axis, Dec from XY-plane
